...
public class regist_with_idfdt_fix {
  ...
  //todo 7 双流connect  <user_id,idfdt,ts>  &  <creative_id,ef_idfdt,country,ts>
   ConnectedStreams<Tuple4<String, String, String, Long>, Tuple3<Long, String, Long>> connect = map_kochava.connect(map_gateway);
        ...
        ElasticsearchSink<Tuple4<Long, String, String, Long>> openSearchSink = openSearchSinkFactory.createOpenSearchSink(config);
        filtercreateid.addSink(openSearchSink);
        env.execute();
    }
}
